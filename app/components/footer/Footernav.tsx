...
const footerLists = [
  ...
  const Footer = () => {
    return (
      <div className="flex mx-auto w-11/12 gap-5 justify-between border-b">
        {footerLists.map((list, index) => (
          <ul className="footer-nav-list w-1/5 mb-10" key={index}>
            <li className="footer-nav-item relative">
              <h2 className="nav-title font-bold text-xl text-white py-2 mt-8 after:bg-red-400 after:absolute after:bottom-0 after:left-0 after:h-px after:w-14 mb-6">{list.title}</h2>
            </li>
            ...
            {list.info && (
              <li className="footer-nav-item flex items-start">
                ...
                <address className="content text-sm">{list.info}</address>
              </li>
            )}
            {list.phone && (
